...
export default function UsersPage() {
  ...
  const roleInfo = (role: string) => ROLES.find(r => r.value === role) || { value: role, label: role, color: "bg-secondary text-foreground border-border" };
  ...
  const formSchema = z.object({
    ...
    role: z.enum(["quality", "supervisor", "agent", "admin"]),
  ...
  const handleChangePw = async (values: ChangePwValues) => {
    ...
    try { await changePasswordMutation.mutateAsync({ id: changePwTarget.id, password: values.password }); changePwForm.reset(); setChangePwTarget(null); } catch {}
  };

  const handleChangeRole = async () => {
    if (!changeRoleTarget || !newRole) return;
    try { await changeRoleMutation.mutateAsync({ id: changeRoleTarget.id, role: newRole }); setChangeRoleTarget(null); setNewRole(""); } catch {}
  };
      ...
      <main className="flex-1 container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
          ...
          <Button onClick={() => setIsFormOpen(true)}
            ...
            <UserPlus className="w-5 h-5 ml-2" />{t("usersAddBtn")}
          </Button>
        </div>
        ...
        <div className="bg-card rounded-2xl border border-border/60 shadow-xl shadow-black/5 overflow-hidden">
          {isLoading ? (
            ...
            <div className="p-16 text-center flex flex-col items-center bg-secondary/10">
              ...
              <Button onClick={() => setIsFormOpen(true)} variant="outline" className="mt-6 border-primary/20 text-primary rounded-xl">{t("usersAddBtn")}</Button>
            </div>
          ) : (
                ...
                <TableBody>
                  {users?.map((user) => (
                    <TableRow key={user.id} className="group hover:bg-secondary/20">
                      <TableCell className="py-4 font-bold">{user.username}</TableCell>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 mt-2">
              <FormField control={form.control} name="username" render={({ field }) => (
                <FormItem className={dir === "rtl" ? "text-right" : "text-left"}>
                  ...
                  <FormControl><Input placeholder={t("usersUsernamePlaceholder")} className="bg-secondary/30 border-secondary h-11" {...field} data-testid="input-username" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="email" render={({ field }) => (
                <FormItem className={dir === "rtl" ? "text-right" : "text-left"}>
                  ...
                  <FormControl><Input type="email" placeholder="user@example.com" className="bg-secondary/30 border-secondary h-11" {...field} data-testid="input-email" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="password" render={({ field }) => (
                <FormItem className={dir === "rtl" ? "text-right" : "text-left"}>
                  ...
                  <FormControl><Input type="password" placeholder={t("usersPwMinLength")} className="bg-secondary/30 border-secondary h-11" {...field} data-testid="input-password" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={form.control} name="role" render={({ field }) => (
                <FormItem className={dir === "rtl" ? "text-right" : "text-left"}>
                  ...
                  <Select onValueChange={field.onChange} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger className="bg-secondary/30 border-secondary h-11" data-testid="select-role"><SelectValue placeholder={t("usersSelectRole")} /></SelectTrigger>
                    </FormControl>
              ...
              <div className="flex gap-3 pt-3 border-t border-border/50">
                <Button type="submit" className="flex-1 bg-primary font-bold h-11" disabled={createMutation.isPending} data-testid="button-submit-user">
                  {createMutation.isPending && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}{t("usersAddSubmit")}
                </Button>
                <Button type="button" variant="outline" onClick={() => setIsFormOpen(false)} className="h-11 px-6" disabled={createMutation.isPending}>{t("cancel")}</Button>
              </div>
            </form>
          </Form>
        </DialogContent>
          ...
          <div className="space-y-4 mt-2">
            <div className="space-y-2">
              ...
              <Select value={newRole} onValueChange={setNewRole}>
                <SelectTrigger className="bg-secondary/30 border-secondary h-11" data-testid="select-new-role"><SelectValue placeholder={t("usersSelectRole")} /></SelectTrigger>
                <SelectContent dir={dir}>
                  {ROLES.map(r => <SelectItem key={r.value} value={r.value}>{r.label}</SelectItem>)}
                </SelectContent>
              </Select>
            ...
            <div className="flex gap-3 pt-3 border-t border-border/50">
              <Button onClick={handleChangeRole} className="flex-1 bg-primary font-bold h-11" disabled={changeRoleMutation.isPending || !newRole} data-testid="button-submit-role">
                {changeRoleMutation.isPending && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}{t("usersSaveRole")}
              </Button>
              <Button variant="outline" onClick={() => setChangeRoleTarget(null)} className="h-11 px-6" disabled={changeRoleMutation.isPending}>{t("cancel")}</Button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
            ...
            <form onSubmit={changePwForm.handleSubmit(handleChangePw)} className="space-y-4 mt-2">
              <FormField control={changePwForm.control} name="password" render={({ field }) => (
                <FormItem className={dir === "rtl" ? "text-right" : "text-left"}>
                  ...
                  <FormControl><Input type="password" placeholder={t("usersPwMinLength")} className="bg-secondary/30 border-secondary h-11" {...field} data-testid="input-new-password" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <FormField control={changePwForm.control} name="confirm" render={({ field }) => (
                <FormItem className={dir === "rtl" ? "text-right" : "text-left"}>
                  ...
                  <FormControl><Input type="password" placeholder={t("usersConfirmPwPlaceholder")} className="bg-secondary/30 border-secondary h-11" {...field} data-testid="input-confirm-password" /></FormControl>
                  <FormMessage />
                </FormItem>
              )} />
              <div className="flex gap-3 pt-3 border-t border-border/50">
                <Button type="submit" className="flex-1 bg-amber-600 hover:bg-amber-700 text-white font-bold h-11" disabled={changePasswordMutation.isPending} data-testid="button-submit-change-pw">
                  {changePasswordMutation.isPending && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}{t("usersSavePw")}
                </Button>
                <Button type="button" variant="outline" onClick={() => setChangePwTarget(null)} className="h-11 px-6">{t("cancel")}</Button>
              </div>
            </form>
          </Form>
        </DialogContent>
      ...
      <AlertDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}>
        <AlertDialogContent dir={dir} className="sm:max-w-[400px]">
          ...
          <AlertDialogFooter className="gap-2 mt-4">
            <AlertDialogAction onClick={(e) => { e.preventDefault(); handleDeleteConfirm(); }} disabled={deleteMutation.isPending} className="bg-destructive text-destructive-foreground flex-1">
              {deleteMutation.isPending && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}{t("usersDeleteConfirm")}
            </AlertDialogAction>
            <AlertDialogCancel disabled={deleteMutation.isPending} className="flex-1 mt-0">{t("usersDeleteCancel")}</AlertDialogCancel>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
